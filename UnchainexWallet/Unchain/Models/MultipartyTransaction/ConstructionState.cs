...
namespace UnchainexWallet.Unchain.Models.MultipartyTransaction;
...
public record ConstructionState : MultipartyTransactionState
{
	...
	public ConstructionState AddInput(Coin coin, OwnershipProof ownershipProof, CoinJoinInputCommitmentData coinJoinInputCommitmentData)
	{
		...
		{
			throw new UnchainProtocolException(UnchainProtocolErrorCode.WrongOwnershipProof);
		}

		if (!StandardScripts.IsStandardScriptPubKey(prevout.ScriptPubKey))
		{
			throw new UnchainProtocolException(UnchainProtocolErrorCode.NonStandardInput);
		}

		if (!Parameters.AllowedInputTypes.Any(x => prevout.ScriptPubKey.IsScriptType(x)))
		{
			throw new UnchainProtocolException(UnchainProtocolErrorCode.ScriptNotAllowed);
		}
		...
		{
			throw new UnchainProtocolException(UnchainProtocolErrorCode.NonUniqueInputs);
		}
	...
	public ConstructionState AddOutputNoMinAmountCheck(TxOut output)
	{
		...
		if (output.IsDust())
		{
			throw new UnchainProtocolException(UnchainProtocolErrorCode.DustOutput);
		}

		if (!StandardScripts.IsStandardScriptPubKey(output.ScriptPubKey))
		{
			throw new UnchainProtocolException(UnchainProtocolErrorCode.NonStandardOutput);
		}
	...
	public SigningState Finalize()
	{
		...
		{
			throw new UnchainProtocolException(UnchainProtocolErrorCode.SizeLimitExceeded, $"Transaction size is {EstimatedVsize} bytes, which exceeds the limit of {Parameters.MaxTransactionSize} bytes.");
		}
		...
		if (EffectiveFeeRate < Parameters.MiningFeeRate)
		{
			...
			throw new UnchainProtocolException(
				UnchainProtocolErrorCode.InsufficientFees,
