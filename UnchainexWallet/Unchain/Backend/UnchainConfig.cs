...
namespace UnchainexWallet.Unchain.Backend;

public class UnchainConfig : ConfigBase
{
	public UnchainConfig() : base("./fakeConfig.for.testing.only.json")
	{
	}

	public UnchainConfig(string filePath) : base(filePath)
	{
	}
	...
	public ExtPubKey CoordinatorExtPubKey { get; init; } = NBitcoinHelpers.BetterParseExtPubKey(Constants.UnchainFallBackCoordinatorExtPubKey);
	...
	public bool UW200CompatibleLoadBalancing { get; init; } = false;

	public double UW200CompatibleLoadBalancingInputSplit { get; init; } = 0.75;
	...
	public static UnchainConfig LoadFile(string filePath)
	{
		try
		{
			using var cfgFile = File.Open(filePath, FileMode.Open, FileAccess.Read);
			var decoder = JsonDecoder.FromStream(Decode.UnchainConfig(filePath));
			var decodingResult = decoder(cfgFile);
			return decodingResult.Match(cfg => cfg, error => throw new InvalidOperationException(error));
		}
		catch (Exception ex)
		{
			var config = new UnchainConfig(filePath);
			config.ToFile();
			Logger.LogInfo($"{nameof(UnchainConfig)} file has been deleted because it was corrupted. Recreated default version at path: `{filePath}`.");
	...
	protected override string EncodeAsJson() =>
		JsonEncoder.ToReadableString(this, Encode.UnchainConfig);
}
