...
public record RoundParameters
{
		...
		Money maxSuggestedAmount)
	{
		return new RoundParameters(
			...
			miningFeeRate,
			maxSuggestedAmount,
			unChainConfig.MinInputCountByRound,
			unChainConfig.MaxInputCountByRound,
			new MoneyRange(unChainConfig.MinRegistrableAmount, unChainConfig.MaxRegistrableAmount),
			new MoneyRange(unChainConfig.MinRegistrableAmount, unChainConfig.MaxRegistrableAmount),
			unChainConfig.AllowedInputTypes,
			unChainConfig.AllowedOutputTypes,
			unChainConfig.StandardInputRegistrationTimeout,
			unChainConfig.ConnectionConfirmationTimeout,
			unChainConfig.OutputRegistrationTimeout,
			unChainConfig.TransactionSigningTimeout,
			unChainConfig.BlameInputRegistrationTimeout,
			unChainConfig.CoordinatorIdentifier,
			unChainConfig.DelayTransactionSigning);
	}
