...
public partial class Arena : IUnchainApiRequestHandler
{
	...
	private async Task<InputRegistrationResponse> RegisterInputCoreAsync(InputRegistrationRequest request, CancellationToken cancellationToken)
	{
		...
		using (await _asyncLock.LockAsync(cancellationToken).ConfigureAwait(false))
		{
			...
			{
				throw new UnchainProtocolException(UnchainProtocolErrorCode.AliceAlreadyRegistered);
			}
			...
			{
				throw new UnchainProtocolException(UnchainProtocolErrorCode.InputNotWhitelisted);
			}
			...
			{
				throw new UnchainProtocolException(UnchainProtocolErrorCode.UneconomicalInput);
			}

			if (alice.TotalInputAmount < round.Parameters.MinAmountCredentialValue)
			{
				throw new UnchainProtocolException(UnchainProtocolErrorCode.NotEnoughFunds);
			}
			if (alice.TotalInputAmount > round.Parameters.MaxAmountCredentialValue)
			{
				throw new UnchainProtocolException(UnchainProtocolErrorCode.TooMuchFunds);
			}

			if (alice.TotalInputVsize > round.Parameters.MaxVsizeAllocationPerAlice)
			{
				throw new UnchainProtocolException(UnchainProtocolErrorCode.TooMuchVsize);
			}
			...
			{
				throw new UnchainProtocolException(UnchainProtocolErrorCode.VsizeQuotaExceeded);
			}
	...
	private async Task<ConnectionConfirmationResponse> ConfirmConnectionCoreAsync(ConnectionConfirmationRequest request, CancellationToken cancellationToken)
	{
		...
		using (await _asyncLock.LockAsync(cancellationToken).ConfigureAwait(false))
		{
			...
			{
				throw new UnchainProtocolException(UnchainProtocolErrorCode.AliceAlreadyConfirmedConnection, $"Round ({request.RoundId}): Alice ({request.AliceId}) already confirmed connection.");
			}
			...
			{
				throw new UnchainProtocolException(UnchainProtocolErrorCode.IncorrectRequestedVsizeCredentials, $"Round ({request.RoundId}): Incorrect requested vsize credentials.");
			}
			...
			{
				throw new UnchainProtocolException(UnchainProtocolErrorCode.IncorrectRequestedAmountCredentials, $"Round ({request.RoundId}): Incorrect requested amount credentials.");
			}
		}
	...
	public async Task<EmptyResponse> RegisterOutputCoreAsync(OutputRegistrationRequest request, CancellationToken cancellationToken)
	{
		using (await _asyncLock.LockAsync(cancellationToken).ConfigureAwait(false))
		{
			...
			if (CoinJoinScriptStore?.Contains(request.Script) is true)
			{
				...
				throw new UnchainProtocolException(UnchainProtocolErrorCode.AlreadyRegisteredScript, $"Round ({request.RoundId}): Already registered script.");
			}
			...
			if (outputScripts.Contains(request.Script))
			{
				...
				throw new UnchainProtocolException(UnchainProtocolErrorCode.AlreadyRegisteredScript, $"Round ({request.RoundId}): Already registered script in some round.");
			}
			...
			if (inputScripts.Contains(request.Script))
			{
				...
				throw new UnchainProtocolException(UnchainProtocolErrorCode.AlreadyRegisteredScript, $"Round ({request.RoundId}): Already registered script some round.");
			}
	...
	public async Task<ReissueCredentialResponse> ReissuanceAsync(ReissueCredentialRequest request, CancellationToken cancellationToken)
	{
		...
		{
			throw new UnchainProtocolException(UnchainProtocolErrorCode.DeltaNotZero, $"Round ({round.Id}): Amount credentials delta must be zero.");
		}

		if (request.RealVsizeCredentialRequests.Delta != 0)
		{
			throw new UnchainProtocolException(UnchainProtocolErrorCode.DeltaNotZero, $"Round ({round.Id}): Vsize credentials delta must be zero.");
		}

		if (request.RealAmountCredentialRequests.Requested.Count() != ProtocolConstants.CredentialNumber)
		{
			throw new UnchainProtocolException(UnchainProtocolErrorCode.WrongNumberOfCreds, $"Round ({round.Id}): Incorrect requested number of amount credentials.");
		}

		if (request.RealVsizeCredentialRequests.Requested.Count() != ProtocolConstants.CredentialNumber)
		{
			throw new UnchainProtocolException(UnchainProtocolErrorCode.WrongNumberOfCreds, $"Round ({round.Id}): Incorrect requested number of weight credentials.");
		}
	...
	public async Task<(Coin coin, int Confirmations)> OutpointToCoinAsync(InputRegistrationRequest request, CancellationToken cancellationToken)
	{
		...
		var txOutResponse = await Rpc.GetTxOutAsync(input.Hash, (int)input.N, includeMempool: true, cancellationToken).ConfigureAwait(false)
			?? throw new UnchainProtocolException(UnchainProtocolErrorCode.InputSpent);
		if (txOutResponse.Confirmations == 0)
		{
			throw new UnchainProtocolException(UnchainProtocolErrorCode.InputUnconfirmed);
		}

		if (txOutResponse.IsCoinBase && txOutResponse.Confirmations <= 100)
		{
			throw new UnchainProtocolException(UnchainProtocolErrorCode.InputImmature);
		}
	...
	private void CheckCoinIsNotBanned(OutPoint input, Round round)
	{
		...
		if (banningTime.Includes(DateTimeOffset.UtcNow))
		{
			...
			throw new UnchainProtocolException(UnchainProtocolErrorCode.InputBanned, exceptionData: new InputBannedExceptionData(banningTime.EndTime));
		}
	}

	private Round GetRound(uint256 roundId) =>
		Rounds.FirstOrDefault(x => x.Id == roundId)
		?? throw new UnchainProtocolException(UnchainProtocolErrorCode.RoundNotFound, $"Round ({roundId}) not found.");
	...
	private Alice GetAlice(Guid aliceId, Round round) =>
		...
		?? throw new UnchainProtocolException(UnchainProtocolErrorCode.AliceNotFound, $"Round ({round.Id}): Alice ({aliceId}) not found.");

	private static bool IsUserCheating(Exception e) =>
		e is UnchainCryptoException || (e is UnchainProtocolException wpe && wpe.ErrorCode.IsEvidencingClearMisbehavior());
}
