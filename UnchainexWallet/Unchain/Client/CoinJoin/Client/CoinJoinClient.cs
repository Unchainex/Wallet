...
using UnchainexWallet.Exceptions;
...
using UnchainexWallet.Unchain.Client.CredentialDependencies;
...
public class CoinJoinClient
{
	...
	private async Task<RoundState> WaitForBlameRoundAsync(uint256 blameRoundId, CancellationToken token)
	{
		...
		if (!roundState.IsBlame && !IsRoundEconomic(roundState.CoinjoinState.Parameters.MiningFeeRate, _roundStatusUpdater.CoinJoinFeeRateMedians, _feeRateMedianTimeFrame))
		{
	...
	private async Task<ImmutableArray<AliceClient>> CreateRegisterAndConfirmCoinsAsync(IEnumerable<SmartCoin> smartCoins, RoundState roundState, CancellationToken cancel)
	{
		...
		async Task<AliceClient?> RegisterInputAsync(SmartCoin coin)
		{
			...
			catch (UnchainProtocolException wpe)
			{
				switch (wpe.ErrorCode)
				{
					case UnchainProtocolErrorCode.RoundNotFound:
						// if the round does not exist then it ended/aborted.
						roundState.LogInfo($"{coin.Coin.Outpoint} arrived too late because the round doesn't exist anymore. Aborting input registrations: '{UnchainProtocolErrorCode.RoundNotFound}'.");
						registrationsCts.Cancel();
						confirmationsCts.Cancel();
						break;

					case UnchainProtocolErrorCode.WrongPhase:
						if (wpe.ExceptionData is WrongPhaseExceptionData wrongPhaseExceptionData)
						{
							roundState.LogInfo($"{coin.Coin.Outpoint} arrived too late. Aborting input registrations: '{UnchainProtocolErrorCode.WrongPhase}'.");
					...
					case UnchainProtocolErrorCode.AliceAlreadyRegistered:
						roundState.LogInfo($"{coin.Coin.Outpoint} was already registered.");
						break;

					case UnchainProtocolErrorCode.AliceAlreadyConfirmedConnection:
						roundState.LogInfo($"{coin.Coin.Outpoint} already confirmed connection.");
						break;

					case UnchainProtocolErrorCode.InputSpent:
					...
					case UnchainProtocolErrorCode.InputBanned or UnchainProtocolErrorCode.InputLongBanned:
					...
					case UnchainProtocolErrorCode.InputNotWhitelisted:
		...
		CancellationToken cancellationToken)
	{
		...
		var tasks = aliceClients.Zip(
			...
			async (aliceClient, scheduledDate) =>
			{
				...
				catch (UnchainProtocolException ex) when (ex.ErrorCode == UnchainProtocolErrorCode.WitnessAlreadyProvided)
				{
