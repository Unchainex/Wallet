...
public static class ImportWalletHelper
{
	...
	public static async Task<KeyManager> ImportWalletAsync(WalletManager walletManager, string walletName, string filePath)
	{
		...
		// Older CC wallet file does not contain 'ColdCardFirmwareVersion' but contains the same keys as Unchainex, so we are checking the number
		...
		KeyManager km = isColdcardJson
			...
			: GetKeyManagerByUnchainexJson(walletManager, filePath, walletFullPath);
	...
	private static KeyManager GetKeyManagerByUnchainexJson(WalletManager manager, string filePath, string walletFullPath)
	{
	...
	private static KeyManager GetKeyManagerByColdcardJson(WalletManager manager, JObject jsonWallet, string walletFullPath)
	{
		...
		// Coldcard 2.1.0 improperly implemented Unchainex skeleton fingerprint at first, so we must reverse byte order.
