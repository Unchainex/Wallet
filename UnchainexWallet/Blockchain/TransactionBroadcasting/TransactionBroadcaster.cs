...
namespace UnchainexWallet.Blockchain.TransactionBroadcasting;
...
public class BackendBroadcaster(IHttpClientFactory httpClientFactory) : IBroadcaster
{
	public async Task<BroadcastingResult> BroadcastAsync(SmartTransaction tx, CancellationToken cancellationToken)
	{
		...
		try
		{
			var unchainexClient = new UnchainexClient(httpClientFactory.CreateClient($"satoshi-broadcast-{tx.GetHash()}"));
			await unchainexClient.BroadcastAsync(tx, cancellationToken).ConfigureAwait(false);
