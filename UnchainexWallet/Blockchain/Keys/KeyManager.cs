...
using UnchainexWallet.CoinJoinProfiles;
using UnchainexWallet.Helpers;
using UnchainexWallet.Io;
...
using UnchainexWallet.Wallets.Slip39;
using static UnchainexWallet.Blockchain.Keys.WpkhOutputDescriptorHelper;
using Decode = UnchainexWallet.Serialization.Decode;
using Encode = UnchainexWallet.Serialization.Encode;
...
public class KeyManager
{
	...
	public IEnumerable<ScriptPubKeySpendingInfo> UnsafeGetSynchronizationInfos(bool isBIP158)
	{
		...
		byte[] GetScriptPubKeyBytes(HdPubKeyInfo hdPubKeyInfo) =>
			isBIP158
				...
				: hdPubKeyInfo.CompressedScriptPubKeyBytes; // Legacy Unchainex backend scripts used to build filters
 	}
	...
	public void SetKeyState(KeyState newKeyState, HdPubKey hdPubKey)
	{
		...
		if (newKeyState is KeyState.Locked or KeyState.Used)
		{
			...
			// This can happen after downgrading to pre-taproot unchainex version the switching back to a supporting
			// version so taproot keys are detected. However, the user has not login yet so taprootextpubkey is
			// not derived yet (because pre-taproot unchainex do not serialize fields that it doesn't know)
			if (keySource is { })
			{
