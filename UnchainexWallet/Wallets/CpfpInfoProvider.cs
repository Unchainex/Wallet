...
public class CpfpInfoProvider : BackgroundService
{
	...
	protected override async Task ExecuteAsync(CancellationToken cancel)
	{
		...
		async Task ScheduleTask(SmartTransaction transaction)
		{
			...
			{
				Logger.LogTrace($"FetchCpfpInfoAsync was unscheduled for {transaction.GetHash()} because Unchainex is shutting down");
				return;
			}
			...
			catch (Exception ex) when (ex is HttpRequestException or OperationCanceledException)
			{
				...
				{
					Logger.LogTrace($"FetchCpfpInfoAsync was canceled for {transaction.GetHash()} because Unchainex is shutting down");
					return;
				}
