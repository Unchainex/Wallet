...
public static class HttpResponseMessageExtensions
{
	...
	public static async Task ThrowRequestExceptionFromContentAsync(this HttpResponseMessage me, CancellationToken cancellationToken)
	{
		...
		if (me.Content is not null)
		{
			...
			{
				{ Type: ProtocolConstants.ProtocolViolationType } => Enum.TryParse<UnchainProtocolErrorCode>(error.ErrorCode, out var code)
					? new UnchainProtocolException(code, error.Description, exceptionData: error.ExceptionData)
					: new NotSupportedException($"Received Unchain protocol exception with unknown '{error.ErrorCode}' error code.\n\tDescription: '{error.Description}'."),
