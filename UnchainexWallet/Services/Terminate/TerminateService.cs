...
namespace UnchainexWallet.Services.Terminate;
...
public class TerminateService
{
	...
	public void Activate()
	{
		...
		{
			// If the debugger is attached and you subscribe to SystemEvents, then on quit Unchainex gracefully stops but never returns from console.
	...
	private void Windows_SystemEvents_SessionEnding(object sender, SessionEndingEventArgs e)
	{
		...
		{
			// This event will only be triggered if you run Unchainex from the published package.
			Logger.LogInfo($"Process termination was requested by the OS, reason '{e.Reason}'.");
			e.Cancel = true;
		}

		// This must be a blocking call because after this the OS will terminate the Unchainex process if it exists.
	...
	private void CurrentDomain_ProcessExit(object? sender, EventArgs e)
	{
		...
		// This must be a blocking call because after this the OS will terminate Unchainex process if exists.
		Terminate();
	}
	...
	public void Terminate()
	{
		...
		Logger.LogSoftwareStopped("Unchainex");
	}
}
