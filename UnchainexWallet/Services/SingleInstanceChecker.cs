...
public enum UnchainexInstanceStatus
{
	Error,
...
{
	private const string UnchainexMagicString = "InBitcoinWeTrust";
	...
	/// <summary>
	/// Creates an object to ensure mutual exclusion of Unchainex instances per Network <paramref name="network"/>.
	/// The solution based on TCP socket.
	/// </summary>
	/// <param name="network">Bitcoin network selected when Unchainex Wallet was started. It will use the port 37129, 37130, 37131 according to network main, test, reg.</param>
	...
	public async Task<UnchainexInstanceStatus> CheckSingleInstanceAsync()
	{
		...
		try
		{
			...
			return singleInstanceResult
				? UnchainexInstanceStatus.NoOtherInstanceIsRunning
				: UnchainexInstanceStatus.AnotherInstanceIsRunning;
		}
		catch (Exception e)
		{
			Logger.LogError(e);
			return UnchainexInstanceStatus.Error;
		}
	}
	...
	/// On macOS this function will never fail if you run Unchainex as a macApp, because mac prevents running the same APP multiple times on OS level.
	/// </summary>
	...
	private async Task<bool> CanRunAsSingleInstanceAsync()
	{
		...
		catch (SocketException ex) when (ex.ErrorCode is 10048 or 48 or 98)
		{
			...
			// It is already used -> another Unchainex is running on this network.
			Logger.LogDebug("Another Unchainex instance is already running.");
		}
...
#pragma warning restore CA2007 // Consider calling ConfigureAwait on the awaited task
		...
		await writer.WriteAsync(UnchainexMagicString.AsMemory(), cts.Token).ConfigureAwait(false);
...
#pragma warning restore CA2007 // Consider calling ConfigureAwait on the awaited task
					...
					if (answer == UnchainexMagicString)
					{
						Logger.LogInfo($"Detected another Unchainex instance.");
						OtherInstanceStarted?.Invoke(this, EventArgs.Empty);
					}
				}
				catch (Exception ex)
				{
					// Somebody connected but it was not another Unchainex instance.
					Logger.LogDebug(ex);
				}
			}
		}
