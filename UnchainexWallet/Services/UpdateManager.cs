...
using UnchainexWallet.Microservices;
using UnchainexWallet.Tor.Http;
...
{
	private const string ReleaseURL = "https://api.github.com/repos/Unchainex/Wallet/releases/latest";
		...
		: base(period)
	{
		...
		_eventBus = eventBus;
		// The feature is disabled on linux at the moment because we install Unchainex Wallet as a Debian package.
	...
	protected override async Task ActionAsync(CancellationToken cancellationToken)
	{
		try
		{
			...
			if (!updateAvailable)
			{
				// After updating Unchainex, remove old installer file.
				Cleanup();
				return;
			}
	...
	private async Task<(string filePath, Version newVersion)> GetInstallerAsync(ReleaseInfo info, CancellationToken cancellationToken)
	{
		...
		await DownloadAndValidateUnchainexSignatureAsync(sha256SumsFilePath, info.AssetDownloadLinks, cancellationToken).ConfigureAwait(false);
	...
	{
		var bytes = await UnchainexSignerHelpers.GetShaComputedBytesOfFileAsync(installerFilePath, cancellationToken).ConfigureAwait(false);
	...
	private async Task<ReleaseInfo> GetLatestReleaseFromGithubAsync(CancellationToken cancellationToken)
	{
		...
		message.Headers.UserAgent.Add(new("UnchainexWallet", "2.0"));
	...
	private async Task DownloadAndValidateUnchainexSignatureAsync(string sha256SumsFilePath, List<string> assetDownloadLinks, CancellationToken cancellationToken)
	{
		var unchainexSigFilePath = Path.Combine(_installerDir, "SHA256SUMS.unchainexsig");
		string sha256SumsUrl = assetDownloadLinks.First(url => url.Contains("SHA256SUMS.asc"));
		string unchainexSigUrl = assetDownloadLinks.First(url => url.Contains("SHA256SUMS.unchainexsig"));
		...
		try
		{
			...
			using HttpRequestMessage signatureRequest = new(HttpMethod.Get, unchainexSigUrl);
			...
			IoHelpers.EnsureContainingDirectoryExists(unchainexSigFilePath);
			File.WriteAllText(unchainexSigFilePath, signatureContent);

			await UnchainexSignerHelpers.VerifySha256SumsFileAsync(sha256SumsFilePath).ConfigureAwait(false);
		}
		catch (HttpRequestException ex)
		{
			...
			{
				message = "Unchainex signature files were not found under the API.";
			}
			else
			{
				message = "Something went wrong while getting Unchainex signature files.";
			}
			throw new InvalidOperationException(message, ex);
		}
		catch (IOException)
		{
			// There's a chance to get IOException when closing Unchainex during stream copying. Throw OperationCancelledException instead.
			cancellationToken.ThrowIfCancellationRequested();
			throw;
		}
	}
