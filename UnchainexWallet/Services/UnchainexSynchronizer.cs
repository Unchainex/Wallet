...
using FiltersResponse = UnchainexWallet.WebClients.Unchainex.FiltersResponse;

namespace UnchainexWallet.Services;

public class UnchainexSynchronizer(
	TimeSpan period,
	...
	: PeriodicRunner(period)
{
	...
	protected override async Task ActionAsync(CancellationToken cancel)
	{
		...
		var unchainexClient = new UnchainexClient(_httpClient, eventBus);
		var lastUsedApiVersion = UnchainexClient.ApiVersion;

		try
		{
			var response = await unchainexClient.GetFiltersAsync(_smartHeaderChain.TipHash, maxFiltersToSync, cancel)
		...
		catch (HttpRequestException ex)
		{
			if (ex.Message.Contains("Not Found"))
			{
				...
				var backendCompatible =
					await CheckBackendCompatibilityAsync(unchainexClient, cancel).ConfigureAwait(false);
				...
				if (backendCompatible && lastUsedApiVersion != UnchainexClient.ApiVersion)
				{
	...
	private static async Task<bool> CheckBackendCompatibilityAsync(UnchainexClient unchainexClient, CancellationToken cancel)
	{
		bool backendCompatible;
		try
		{
			backendCompatible = await unchainexClient.CheckUpdatesAsync(cancel).ConfigureAwait(false);
		}
