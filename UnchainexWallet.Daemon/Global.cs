...
using UnchainexWallet.BitcoinP2p;
...
public class Global
{
	...
	public Global(string dataDir, string configFilePath, Config config)
	{
		...
		HostedServices.Register<UnchainexSynchronizer>(() => new UnchainexSynchronizer(requestInterval, maxFiltersToSync, BitcoinStore, BackendHttpClientFactory, EventBus), "Unchainex Synchronizer");
	...
	private async Task StartRpcServerAsync(TerminateService terminateService, CancellationToken cancel)
	{
		...
		{
			var unchainexJsonRpcService = new Rpc.UnchainexJsonRpcService(global: this);
			RpcServer = new JsonRpcServer(unchainexJsonRpcService, jsonRpcServerConfig, terminateService);
			try
			{
	...
	private void RegisterCoinJoinComponents(Uri coordinatorUri)
	{
		...
		var unChainStatusProvider =  new UnchainHttpApiClient("satoshi-coordination", CoordinatorHttpClientFactory);
		HostedServices.Register<RoundStateUpdater>(() => new RoundStateUpdater(TimeSpan.FromSeconds(10), unChainStatusProvider), "Round info updater");

		Func<string, UnchainHttpApiClient> unChainHttpClientFactory = (identity) => new UnchainHttpApiClient(identity, CoordinatorHttpClientFactory!);
		...
		HostedServices.Register<CoinJoinManager>(() => new CoinJoinManager(WalletManager, HostedServices.Get<RoundStateUpdater>(), unChainHttpClientFactory, coinJoinConfiguration, CoinPrison, EventBus), "CoinJoin Manager");
	}
