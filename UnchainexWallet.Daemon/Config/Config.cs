...
public class Config
{
	...
	public Config(PersistentConfig persistentConfig, string[] cliArgs)
	{
		...
		Data = new()
		{
			...
			[nameof(TerminateTorOnExit)] = (
				"Stop the Tor process when Unchainex is closed",
				GetBoolValue("TerminateTorOnExit", PersistentConfig.TerminateTorOnExit, cliArgs)),
			[ nameof(DownloadNewVersion)] = (
				"Automatically download any new released version of Unchainex",
			...
			[ nameof(BlockOnlyMode)] = (
				"Unchainex listens only for blocks and not for transactions",
			...
			[ nameof(DropUnconfirmedTransactionsAfterDays) ] = (
				"The number of days that unconfirmed wallet transactions will be remembered by Unchainex before dropping them",
	...
	public static string DataDir { get; } = GetStringValue(
		"datadir",
		EnvironmentHelpers.GetDataDir(Path.Combine("UnchainexWallet", "Client")),
	...
	public bool TryGetCoordinatorUri([NotNullWhen(true)] out Uri? coordinatorUri)
	{
		try
		{
			...
			return coordinatorUri.Host != "api.unchainex.org";
		}
	...
	{
		string envKey = $"UNCHAINEX-{key.ToUpperInvariant()}";
