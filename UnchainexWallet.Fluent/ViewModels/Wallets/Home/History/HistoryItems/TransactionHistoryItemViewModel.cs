...
public partial class TransactionHistoryItemViewModel : HistoryItemViewModelBase
{
	...
	private async Task OnSpeedUpTransactionAsync(TransactionModel transaction, CancellationToken cancellationToken)
	{
		...
		{
			Logger.LogError(ex);
			UiContext.Navigate().To().ShowErrorDialog(ex.ToUserFriendlyString(), "Speed Up failed", "Unchainex could not initiate the transaction speed up process.");
		}
	}
	...
	private void OnCancelTransaction(TransactionModel transaction)
	{
		...
		{
			Logger.LogError(ex);
			UiContext.Navigate().To().ShowErrorDialog(ex.ToUserFriendlyString(), "Cancel failed", "Unchainex could not initiate the cancelling process.");
		}
	}
}
