...
using UnchainexWallet.Fluent.Views.Wallets.Transactions.Outputs.Cells;
using UnchainexWallet.Fluent.Views.Wallets.Transactions.Outputs.Columns;
...
public static class OutputsCoinListDataGridSource
{
	...
	private static IColumn<OutputsCoinListItem> AmountColumn()
	{
		return new TemplateColumn<OutputsCoinListItem>(
			...
			{
				CompareAscending = Sort<OutputsCoinListItem>.Ascending(x => x.Amount.Btc),
				CompareDescending = Sort<OutputsCoinListItem>.Descending(x => x.Amount.Btc)
			});
	}
