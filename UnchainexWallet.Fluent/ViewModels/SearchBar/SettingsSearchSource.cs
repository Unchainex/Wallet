...
public class SettingsSearchSource : ReactiveObject, ISearchSource
{
	...
	private IEnumerable<ISearchItem> GetSettingsItems()
	{
		...
		yield return new ContentSearchItem(content: Setting(selector: x => x.RunOnSystemStartup), name: "Run Unchainex when computer starts", category: "Settings", keywords: new List<string>() { "startup", "boot" }, icon: "nav_settings_regular", isEnabled) { IsDefault = false, Priority = 5 };
