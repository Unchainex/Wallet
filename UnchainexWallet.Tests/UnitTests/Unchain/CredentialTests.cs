...
namespace UnchainexWallet.Tests.UnitTests.Unchain;
...
public class CredentialTests
{
	...
	public void CorrectRangeProof()
	{
		...
		var client = new UnchainClient(sk.ComputeCredentialIssuerParameters(), rnd, 4300000000000);
		...
		client = new UnchainClient(sk.ComputeCredentialIssuerParameters(), rnd, 4400000000001);
		...
		public void CredentialIssuance()
		{
			...
			{
				var client0 = new UnchainClient(sk.ComputeCredentialIssuerParameters(), rnd, 4300000000000);
		...
		public void InvalidCredentialRequests()
		{
			...
			var issuer = new CredentialIssuer(sk, rnd, 4300000000000);
			{
				...
				var ex = Assert.Throws<UnchainCryptoException>(() => issuer.PrepareResponse(invalidCredentialRequest));
				Assert.Equal(UnchainCryptoErrorCode.InvalidNumberOfPresentedCredentials, ex.ErrorCode);
				...
				ex = Assert.Throws<UnchainCryptoException>(() => issuer.PrepareResponse(invalidCredentialRequest));
				Assert.Equal(UnchainCryptoErrorCode.InvalidNumberOfPresentedCredentials, ex.ErrorCode);
				...
				ex = Assert.Throws<UnchainCryptoException>(() => issuer.PrepareResponse(invalidCredentialRequest));
				Assert.Equal(UnchainCryptoErrorCode.InvalidNumberOfRequestedCredentials, ex.ErrorCode);
				...
				ex = Assert.Throws<UnchainCryptoException>(() => issuer.PrepareResponse(invalidCredentialRequest));
				Assert.Equal(UnchainCryptoErrorCode.InvalidBitCommitment, ex.ErrorCode);
			}

			{
				...
				var ex = Assert.Throws<UnchainCryptoException>(() => issuer.PrepareResponse(invalidCredentialRequest));
				Assert.Equal(UnchainCryptoErrorCode.CoordinatorReceivedInvalidProofs, ex.ErrorCode);
			}

			{
				...
				var ex = Assert.Throws<UnchainCryptoException>(() => issuer.PrepareResponse(validCredentialRequest));
