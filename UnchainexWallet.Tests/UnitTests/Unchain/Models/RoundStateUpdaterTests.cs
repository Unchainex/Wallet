...
public class RoundStateUpdaterTests
{
	...
	{
		var roundState1 = RoundState.FromRound(UnchainFactory.CreateRound(cfg: new()));
		var roundState2 = RoundState.FromRound(UnchainFactory.CreateRound(cfg: new()));
		...
		var apiClient = new UnchainHttpApiClient("identity", mockHttpClientFactory);
	...
	{
		var roundState = RoundState.FromRound(UnchainFactory.CreateRound(cfg: new()));
