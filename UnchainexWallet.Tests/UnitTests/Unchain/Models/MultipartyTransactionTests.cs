...
public class MultipartyTransactionTests
{
	...
	private static readonly RoundParameters DefaultParameters = UnchainFactory.CreateRoundParameters(new()
	{
	...
	private static readonly CoinJoinInputCommitmentData CommitmentData = UnchainFactory.CreateCommitmentData();

	private static void ThrowsProtocolException(UnchainProtocolErrorCode expectedError, Action action) =>
		Assert.Equal(expectedError, Assert.Throws<UnchainProtocolException>(action).ErrorCode);
	...
	public void TwoPartiesNoFees()
	{
		...
		(var alice1Coin, var alice1OwnershipProof) = UnchainFactory.CreateCoinWithOwnershipProof(key1);
		(var alice2Coin, var alice2OwnershipProof) = UnchainFactory.CreateCoinWithOwnershipProof(key2);
	...
	{
		(var coin, var ownershipProof) = UnchainFactory.CreateCoinWithOwnershipProof();
	...
	public void WitnessValidation()
	{
		...
		{
			ThrowsProtocolException(UnchainProtocolErrorCode.WrongCoinjoinSignature, () => noFeeTx.AddWitness(invalidSignature.Index, invalidSignature.Input.WitScript));
		}
		...
		ThrowsProtocolException(UnchainProtocolErrorCode.WitnessAlreadyProvided, () => alice1Sig.AddWitness(alice1SignedInput.Index, alice1SignedInput.Input.WitScript));
	}
	[Fact]
	...
	public void FeeRateValidation()
	{
		...
		var bob2 = new TxOut(Money.Coins(1), alice2Coin.ScriptPubKey);
		ThrowsProtocolException(UnchainProtocolErrorCode.InsufficientFees, () => withOutput.AddOutput(bob2).Finalize());
	...
	public void NoDuplicateInputs()
	{
		(var coin, var ownershipProof) = UnchainFactory.CreateCoinWithOwnershipProof();
		var state = new ConstructionState(DefaultParameters).AddInput(coin, ownershipProof, CommitmentData);
		ThrowsProtocolException(UnchainProtocolErrorCode.NonUniqueInputs, () => state.AddInput(coin, ownershipProof, CommitmentData));
	...
	public void OnlyAllowedInputTypes()
	{
		...
		(var coin, var ownershipProof) = UnchainFactory.CreateCoinWithOwnershipProof();
		ThrowsProtocolException(UnchainProtocolErrorCode.ScriptNotAllowed, () => legacyOnly.AddInput(coin, ownershipProof, CommitmentData));
	}
	...
	public void InputAmountRanges()
	{
		...
		ThrowsProtocolException(UnchainProtocolErrorCode.NotEnoughFunds, () => above.AddInput(coin, ownershipProof, CommitmentData));
		ThrowsProtocolException(UnchainProtocolErrorCode.TooMuchFunds, () => below.AddInput(coin, ownershipProof, CommitmentData));
	...
	{
		(var alice1Coin, var alice1OwnershipProof) = UnchainFactory.CreateCoinWithOwnershipProof(amount: new Money(1000L));
		(var alice2Coin, var alice2OwnershipProof) = UnchainFactory.CreateCoinWithOwnershipProof(amount: new Money(2000L));
		...
		ThrowsProtocolException(UnchainProtocolErrorCode.UneconomicalInput, () => state.AddInput(alice1Coin, alice1OwnershipProof, CommitmentData));
	...
	public void NoNonStandardOutput()
	{
		...
		ThrowsProtocolException(UnchainProtocolErrorCode.NonStandardOutput, () => state.AddOutput(sha256Bounty));
	}
	...
	public void OnlyAllowedOutputTypes()
	{
		...
		ThrowsProtocolException(UnchainProtocolErrorCode.ScriptNotAllowed, () => legacyOnly.AddOutput(new TxOut(Money.Coins(1), p2wpkh)));
	}
	...
	public void OutputAmountRanges()
	{
		...
		var p2wpkh = BitcoinFactory.CreateScript();
		ThrowsProtocolException(UnchainProtocolErrorCode.NotEnoughFunds, () => costly.AddOutput(new TxOut(Money.Coins(1), p2wpkh)));
		ThrowsProtocolException(UnchainProtocolErrorCode.TooMuchFunds, () => costly.AddOutput(new TxOut(Money.Coins(12), p2wpkh)));
	...
	public void NoDustOutputs()
	{
		...
		var p2wpkh = BitcoinFactory.CreateScript();
		ThrowsProtocolException(UnchainProtocolErrorCode.DustOutput, () => state.AddOutput(new TxOut(new Money(1L), p2wpkh)));
		ThrowsProtocolException(UnchainProtocolErrorCode.DustOutput, () => state.AddOutput(new TxOut(new Money(293L), p2wpkh)));
	...
	public void FeeTests(int inputCount, int outputCount, string feeRateString)
	{
		...
		var parameters = UnchainFactory.CreateRoundParameters(new()
		{
		...
		{
			using Key key = new();
			(var aliceCoin, var aliceOwnershipProof) = UnchainFactory.CreateCoinWithOwnershipProof(key);
