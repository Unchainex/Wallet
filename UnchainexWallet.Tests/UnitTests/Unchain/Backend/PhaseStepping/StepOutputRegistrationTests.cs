...
public class StepOutputRegistrationTests
{
	...
	public async Task AllBobsRegisteredAsync()
	{
		...
		var (keyChain, coin1, coin2) = UnchainFactory.CreateCoinKeyPairs();

		var mockRpc = UnchainFactory.CreatePreconfiguredRpcClient(coin1.Coin, coin2.Coin);
	...
	public async Task DiffTooSmallToBlameAsync()
	{
		...
		var extraAlice = UnchainFactory.CreateAlice(round.Parameters.MiningFeeRate.GetFee(Constants.P2wpkhInputVirtualSize) + txParameters.AllowedOutputAmounts.Min - new Money(1L), round);
		extraAlice.ReadyToSign = true;
		round.Alices.Add(extraAlice);
		round.CoinjoinState = round.Assert<ConstructionState>().AddInput(extraAlice.Coin, extraAlice.OwnershipProof, UnchainFactory.CreateCommitmentData(round.Id));
	...
	public async Task SomeBobsReusingAddressAsync()
	{
		...
		var (keyChain1, coin1a, coin1b) = UnchainFactory.CreateCoinKeyPairs(keyManager1);
		var (keyChain2, coin2a, coin2b) = UnchainFactory.CreateCoinKeyPairs(keyManager2);

		var mockRpc = UnchainFactory.CreatePreconfiguredRpcClient(coin1a.Coin, coin1b.Coin, coin2a.Coin, coin2b.Coin);
		using Arena arena = await ArenaBuilder.From(cfg).With(mockRpc).CreateAndStartAsync();

		// Get the round.
		await arena.TriggerAndWaitRoundAsync(token);
		var round1 = Assert.Single(arena.Rounds);
		var arenaClient1 = UnchainFactory.CreateArenaClient(arena);
		var round2 = UnchainFactory.CreateRound(UnchainFactory.CreateRoundParameters(cfg));
		...
		var arenaClient2 = UnchainFactory.CreateArenaClient(arena, round2);
		...
		var bob2a = Task.Run(async () =>
		{
			...
			do
			{
				...
				catch (UnchainProtocolException)
				{
				}
