...
public class UnchainHttpApiIntegrationTests : IClassFixture<UnchainApiApplicationFactory<Startup>>
{
	private readonly UnchainApiApplicationFactory<Startup> _apiApplicationFactory;
	private readonly ITestOutputHelper _output;

	public UnchainHttpApiIntegrationTests(UnchainApiApplicationFactory<Startup> apiApplicationFactory, ITestOutputHelper output)
	{
	...
	public async Task RegisterSpentOrInNonExistentCoinAsync()
	{
		...
		var ownershipProof = UnchainFactory.CreateOwnershipProof(signingKey, round.Id);
	...
	public async Task RegisterBannedCoinAsync()
	{
		...
		using var signingKey = new Key();
		var coin = UnchainFactory.CreateCoin(signingKey);
		...
		var httpClient = _apiApplicationFactory.WithWebHostBuilder(builder =>
			builder.ConfigureServices(services =>
			{
				...
				var prison = UnchainFactory.CreatePrison();
	...
	public async Task SoloCoinJoinTestAsync(long[] amounts)
	{
		...
		var httpClient = _apiApplicationFactory.WithWebHostBuilder(builder =>
			...
			.ConfigureServices(services =>
			{
				...
				// services to build everything (Unchain controller, arena, etc)
				services.AddSingleton(s => new UnchainConfig
				{
		...
		var apiClient = _apiApplicationFactory.CreateUnchainHttpApiClient(httpClient);
		...
		var coinJoinClient = UnchainFactory.CreateTestCoinJoinClient(_ => apiClient, keyManager, roundStateUpdater);
	...
	public async Task FailToRegisterOutputsCoinJoinTestAsync()
	{
		...
		var httpClient = _apiApplicationFactory.WithWebHostBuilder(builder =>
			...
			.ConfigureServices(services =>
			{
				...
				// services to build everything (Unchain controller, arena, etc)
				services.AddSingleton(_ => new UnchainConfig
				{
				...
				// the server will prevent the registration and fail with a UnchainProtocolError.
	...
	public async Task CoinJoinWithBlameRoundTestAsync(long[] amounts)
	{
		...
		// Create the coinjoin client
		var apiClient = _apiApplicationFactory.CreateUnchainHttpApiClient(app.CreateClient());
		...
		var nonSigningApiClient = _apiApplicationFactory.CreateUnchainHttpApiClient(nonSigningHttpClientMock);

		var coinJoinClient = UnchainFactory.CreateTestCoinJoinClient(_ => apiClient, keyManager1, roundStateUpdater);
		var badCoinJoinClient = UnchainFactory.CreateTestCoinJoinClient(_ => nonSigningApiClient, keyManager2, roundStateUpdater);
		...
		double delayInjectorMonkeyAggressiveness)
	{
		...
		var app = _apiApplicationFactory.WithWebHostBuilder(builder =>
			builder.ConfigureServices(services =>
			{
				...
				// services to build everything (Unchain controller, arena, etc)
				services.AddSingleton<IRPCClient>(s => rpc);
				services.AddSingleton(s => new UnchainConfig(Path.GetTempFileName())
				{
		...
		var apiClient = new UnchainHttpApiClient("", new MockHttpClientFactory {OnCreateClient = _ => httpClientWrapper});
