...
namespace UnchainexWallet.Tests.UnitTests.Wallet;
...
public class WalletBuilder : IAsyncDisposable
{
	...
	public async Task<UnchainexWallet.Wallets.Wallet> CreateRealWalletBasedOnTestWalletAsync(TestWallet wallet, int? minGapLimit = null)
	{
		...
		var serviceConfiguration = new ServiceConfiguration(new UriEndPoint(new Uri("http://www.nomatter.dontcare")), Money.Coins(UnchainexWallet.Helpers.Constants.DefaultDustThreshold));
