...
public class SingleInstanceCheckerTests
{
	...
	public async Task SingleInstanceTestsAsync()
	{
		...
		Assert.Equal(UnchainexInstanceStatus.NoOtherInstanceIsRunning, status);
		...
		Assert.Equal(UnchainexInstanceStatus.AnotherInstanceIsRunning, status);
	...
	public async Task OtherInstanceStartedTestsAsync()
	{
		...
		try
		{
			...
			for (int i = 0; i < 2; i++)
			{
				...
				Assert.Equal(UnchainexInstanceStatus.AnotherInstanceIsRunning, secondInstanceStatus);
			}
			...
			Assert.Equal(UnchainexInstanceStatus.AnotherInstanceIsRunning, statusAfterPortScan);
