...
using UnchainexWallet.Tests.UnitTests;
using UnchainexWallet.Tests.UnitTests.Unchain;
...
public static class UnchainFactory
{
	private static string CoordinatorIdentifier = new UnchainConfig().CoordinatorIdentifier;
	...
	public static RoundParameters CreateRoundParameters(UnchainConfig cfg) =>
	...
	public static Round CreateRound(UnchainConfig cfg) =>
	...
	{
		(var amClient, var vsClient, _, _, _, _) = CreateUnchainClientsAndIssuers(round);
	...
	public static (
		UnchainClient amountClient,
		UnchainClient vsizeClient,
	...
	private static (IEnumerable<Credential>, IEnumerable<Credential>) EnsureZeroCredentials(UnchainClient amClient, UnchainClient vsClient, CredentialIssuer amIssuer, CredentialIssuer vsIssuer)
	{
	...
	{
		var (amClient, vsClient, _, _, amZeroCredentials, vsZeroCredentials) = CreateUnchainClientsAndIssuers(round);
	...
	public static ConnectionConfirmationRequest CreateConnectionConfirmationRequest(Round round)
	{
		...
		var (amClient, vsClient, _, _, _, _) = CreateUnchainClientsAndIssuers(round);
		var (zeroAmountCredentialRequest, _) = amClient.CreateRequestForZeroAmount();
		var (zeroVsizeCredentialRequest, _) = vsClient.CreateRequestForZeroAmount();
	...
	{
		var (amClient, vsClient, amIssuer, vsIssuer, amZeroCredentials, vsZeroCredentials) = CreateUnchainClientsAndIssuers(round);
	...
	public static BlameRound CreateBlameRound(Round round, UnchainConfig cfg)
	{
	...
	public static CoinJoinClient CreateTestCoinJoinClient(
		Func<string, IUnchainApiRequestHandler> apiClientFactory,
	...
	public static RoundParameterFactory CreateRoundParametersFactory(UnchainConfig cfg, Network network, int maxVsizeAllocationPerAlice)
	{
	...
	internal static UnchainConfig CreateUnchainConfig()
	{
		return new UnchainConfig
		{
...
{
	public TesteableCoinJoinClient(Func<string, IUnchainApiRequestHandler> arenaRequestHandlerFactory, IKeyChain keyChain, OutputProvider outputProvider, RoundStateUpdater roundStatusUpdater, CoinJoinCoinSelector coinJoinCoinSelector, CoinJoinConfiguration coinJoinConfiguration, LiquidityClueProvider liquidityClueProvider, TimeSpan feeRateMedianTimeFrame = default, TimeSpan doNotRegisterInLastMinuteTimeLimit = default) : base(arenaRequestHandlerFactory, keyChain, outputProvider, roundStatusUpdater, coinJoinCoinSelector, coinJoinConfiguration, liquidityClueProvider, feeRateMedianTimeFrame, doNotRegisterInLastMinuteTimeLimit)
	{
	}
