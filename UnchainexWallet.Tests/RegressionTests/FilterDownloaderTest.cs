...
public class FilterDownloaderTest : IClassFixture<RegTestFixture>
{
	...
	public async Task FilterDownloaderTestAsync()
	{
		...
		using UnchainexSynchronizer synchronizer = new(period: TimeSpan.FromSeconds(1), 1000, bitcoinStore, RegTestFixture.BackendHttpClientFactory, setup.EventBus);
		try
		{
			...
			while ((filterCount = bitcoinStore.SmartHeaderChain.HashCount) < blockCount)
			{
				if (times > 500) // 30 sec
				{
					throw new TimeoutException($"{nameof(UnchainexSynchronizer)} test timed out. Needed filters: {blockCount}, got only: {filterCount}.");
				}
			...
			while ((filterCount = bitcoinStore.SmartHeaderChain.HashCount) < blockCount + 10)
			{
				if (times > 500) // 30 sec
				{
					throw new TimeoutException($"{nameof(UnchainexSynchronizer)} test timed out. Needed filters: {blockCount + 10}, got only: {filterCount}.");
				}
				await Task.Delay(100);
				times++;
			}
