...
public class BuildTransactionValidationsTest : IClassFixture<RegTestFixture>
{
	...
	public async Task BuildTransactionValidationsTestAsync()
	{
		...
		// 3. Create unchainex synchronizer service.
		using UnchainexSynchronizer synchronizer = new(period: TimeSpan.FromSeconds(3), 10000, bitcoinStore, RegTestFixture.BackendHttpClientFactory, setup.EventBus);
		...
		try
		{
			...
			await synchronizer.StartAsync(CancellationToken.None); // Start unchainex synchronizer service.
