...
#  release.sh                                                                        #
#                                                                                    #
#  This script builds the `UnchainexWallet.Fluent.Desktop` for all the supported     #
...
PACKAGE_FILE_NAME_PREFIX="Unchainex-$VERSION"
...
# Copy icon files
for ICON_FILE in ./Contrib/Assets/UnchainexLogo*.png; do
...
Priority: optional
Section: utils
Maintainer: Unchainex Wallet Team
Version: ${VERSION}
Homepage: https://unchainex.org
Vcs-Git: git://github.com/Unchainex/Wallet.git
Vcs-Browser: https://github.com/Unchainex/Wallet
Architecture: amd64
...
INSTALL_DIR="${USR_LOCAL_BIN_DIR}/unchainexwallet"
...
Type=Application
Name=Unchainex Wallet
StartupWMClass=Unchainex Wallet
...
Keywords=bitcoin;wallet;crypto;blockchain;unchainex;privacy;anon;awesome;"
...
cp -r $BUILD_DIR/linux-x64 $DEBIAN_BIN/unchainexwallet
...
sudo chmod -R 0655 ${DEBIAN_BIN}/unchainexwallet
sudo chmod 0755 ${DEBIAN_BIN}/unchainexwallet/${EXECUTABLE_NAME}{,d}
...
if [[ "$PACKAGE_COORDINATOR" == "yes" ]]; then
  ...
  # Remove execution to everything except for executables and their wrapper scripts
  sudo chmod 0755 ${DEBIAN_BIN}/unchainexwallet/${BACKEND_EXECUTABLE_NAME}
  sudo chmod 0755 ${DEBIAN_BIN}/${BACKEND_EXECUTABLE_NAME}
  sudo chmod 0755 ${DEBIAN_BIN}/unchainexwallet/${COORDINATOR_EXECUTABLE_NAME}
...
WIX_BIN_DIR="$WIX/bin"
WINDOWS_INSTALLER_DIR="UnchainexWallet.WindowsInstaller"
...
"$SIGNTOOL" Sign -d 'Unchainex Wallet' -f 'Certificate.pfx'  -p "$SIGNING_CERTIFICATE_PASSWORD" -t 'http://timestamp.digicert.com' -v "$PACKAGES_DIR/$PACKAGE_FILE_NAME_PREFIX.msi"
fi
...
for OSX_ZIP_PACKAGE in $PACKAGES_DIR/Unchainex*macOS*.zip; do
# Combine paths
...
<dict>
	...
	<string>UnchainexLogo.icns</string>
	...
	<string>unchainex.unchainexwallet</string>
...
#Add logo
cp ./Contrib/Assets/UnchainexLogo.icns "$APP_RES_PATH/UnchainexLogo.icns"
...
P12_PATH="MacP12.p12"
PROFILE_NAME="UnchainexNotarize"
...
if ! codesign -dv --verbose=4 "$APP_PATH" 2>&1 | grep -q 'Authority=Developer ID Application: Unchainex Ltd.'; then
...
xcrun notarytool submit --wait --apple-id "${MAC_APPLEID}" -p "UnchainexNotarize" "$APP_NOTARIZE_FILE_PATH"
...
DMG_UNZIPPED_FILE_PATH="$OSX_BUILD_DIR/Unchainex.tmp.dmg"
DMG_FILE_PATH="$DMG_PATH/$PACKAGE_FILE_NAME_PREFIX$DMG_ARCH_NAME.dmg"

# Create the dmg
ln -s /Applications "$DMG_PATH"
hdiutil create "$DMG_UNZIPPED_FILE_PATH" -ov -volname "Unchainex Wallet" -fs HFS+ -srcfolder "$DMG_PATH"
hdiutil convert "$DMG_UNZIPPED_FILE_PATH" -format UDZO -o "$DMG_FILE_PATH"

codesign $SIGN_ARGUMENTS "$DMG_FILE_PATH"

if ! codesign -dv --verbose=4 "$DMG_FILE_PATH" 2>&1 | grep -q 'Authority=Developer ID Application: Unchainex Ltd.'; then
...
if ! xcrun notarytool submit --wait --apple-id "${MAC_APPLEID}" -p "UnchainexNotarize" "$DMG_FILE_PATH" | grep -q "Accepted"; then
...
let unchainexPrivateKey = Key.Parse(args[3], Network.Main)
args[2]
|> File.ReadAllBytes
|> SHA256.HashData
|> uint256
|> unchainexPrivateKey.Sign
|> _.ToDER()
|> Convert.ToBase64String
|> Console.WriteLine
" > signer.fsx
dotnet fsi signer.fsx SHA256SUMS.asc $SIGNING_UNCHAINEX_KEY > SHA256SUMS.unchainexsig
rm signer.fsx
...
if [ "$RELEASE_NOTE" = "yes" ]; then
  sed -e "s/{version}/$VERSION/g" \
      -e "/{highlights}/r ./UnchainexWallet/Announcements/ReleaseHighlights.md" \
      -e "/{highlights}/d" \
