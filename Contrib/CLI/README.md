...
A bash script for effortless interaction with the Unchainex RPC Server.

USAGE:

```bash
$ ./ucli.sh [-wallet=<WALLET-NAME>] command [ARGS,...]
```
The supported RPC commands are listed in the [documentation](https://docs.unchainex.org/using-unchainex/RPC.html).
...
```bash
$ ./ucli.sh -wallet=MyWallet getwalletinfo

{
  "walletName": "MyWallet",
  "walletFile": "/home/ricardo/.unchainexwallet/client/Wallets/MyWallet.json",
  "state": "Started",
...
```bash
$ ./ucli.sh -wallet=MyWallet listkeys | head -10
...
```bash
$ ./ucli.sh -wallet=MyWallet gethistory | head -10
...
```bash
$ ./ucli.sh -wallet=MyWallet getnewaddress "Ricardo"
